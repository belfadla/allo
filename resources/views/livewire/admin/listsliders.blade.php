						...
						<table class="table hover multiple-select-row data-table-export nowrap">
							...
							@foreach($list as $service)
								...
								<div class="dropdown">
                                    ...
                                    <form action="{{route('slider.destroy',$service->id)}}" method="POST">
                                    {{ csrf_field() }}
                                    {{method_field('DELETE')}}
           
                                        @if($service->status==1)
                                        <a class="dropdown-item" href="{{URL::to('slider/unactivate/'.$service->id)}}">
                                        <i class="icon-copy dw dw-thumb-down"></i> desactiver 
                            </a>
                            @else
                            <a  class="dropdown-item" href="{{URL::to('slider/activate/'.$service->id)}}">
